...
-- ----------------------------------------------------------------------------
-- 6. start_pilot_trip — batch start all assigned orders for one pilot
-- ----------------------------------------------------------------------------
...
DECLARE
  ...
  r          record;
BEGIN
  v_cached := public._claim_mutation(p_mutation_id, 'start_pilot_trip', 'delivery', p_pilot_id::text);
  IF v_cached IS NOT NULL THEN
    RETURN v_cached;
  END IF;
  ...
  FOR r IN
    ...
    WHERE delivery_id = p_pilot_id
      AND status = ANY (public._assigned_order_statuses())
    FOR UPDATE
  LOOP
    UPDATE public.orders SET status = v_status_active WHERE id = r.id;
    PERFORM public._log_order_status(r.id, r.status, v_status_active, 'start_pilot_trip');
    v_count := v_count + 1;
  END LOOP;

  IF v_count = 0 THEN
    RAISE EXCEPTION 'No assigned orders to start for pilot %', p_pilot_id USING ERRCODE = 'P0001';
  END IF;
  ...
  v_result := jsonb_build_object(
    'pilot_id', p_pilot_id,
    'orders_started', v_count,
    'state', 'on_delivery'
  );

  PERFORM public._finish_mutation(p_mutation_id, 'start_pilot_trip', 'delivery', p_pilot_id::text, v_result);
  RETURN v_result;
END;
$$;
...
BEGIN
  ...
  IF v_pilot_id IS NOT NULL THEN
    ...
    WHERE delivery_id = v_pilot_id
      AND id <> p_order_id
      AND status = ANY (public._active_delivery_statuses());
...
GRANT EXECUTE ON FUNCTION public.start_pilot_trip(bigint, uuid) TO anon, authenticated, service_role;
